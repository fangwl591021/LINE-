  ...
  const [countRow, mineRow] = await Promise.all([
    ...
    actorUserId
      ? db.prepare(`
          ...
          LIMIT 1
        `).bind(handle, actorUserId).first()
      : Promise.resolve(null)
  ]);

  return {
    likeCount: countValue(countRow),
    likedByMe: Boolean(mineRow)
  };
}
...
export async function toggleExchangeZoneLike(db, payload, actor) {
  ...
  if (!post) {
    return { success: false, error: '找不到可按讚的交流內容', code: 'EXCHANGE_LIKE_POST_NOT_AVAILABLE' };
  }
