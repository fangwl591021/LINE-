...
const inviter = has('inviter_user_id', 'inviter_uid', 'referrer_id', 'introducer_id');
const recognized = has('recognized_user_id', 'recognized_person_uid', 'recognized_uid');
...
const claimEvent = has('claim_event_id', 'claim_audit_id', 'claimed_at');

const checks = [];
function add(code, severity, description, sql, required) {
  const missing = required.filter((x) => !x);
  if (missing.length) {
    checks.push({ code, severity, description, status: 'not_evaluable', missingAliases: required.map((x, i) => x ? null : i).filter((x) => x !== null) });
    return;
  }
  if (!localDbPath) {
  ...
  checks.push({
    ...
    severity,
    description,
    status: rows.length ? 'finding' : 'clear',
    count: rows.length,
    samples: rows.slice(0, 20).map((r) => Object.fromEntries(Object.entries(r).map(([k, v]) => [k, /id|uid|phone|email|name/i.test(k) ? mask(v) : v])))
  });
}

const activePredicate = !active ? '1=1' : active === 'status' ? `${active} NOT IN ('deleted','merged','inactive')` : `${active} = 1`;
const personalPredicate = source ? `${source} IN ('self_profile','video_profile','line_generated','self_upload','claimed_personal','claimed')` : '1=1';
const contactPredicate = source ? `${source} IN ('private_import','ocr_scan','referral_placeholder','legacy_import','claimed_contact')` : '1=1';
const canonicalUid = owner || profile || line || bound;
...
add('A06','critical','Same UID has multiple standard candidates',`SELECT COALESCE(${owner},${profile},${line}) uid_key, COUNT(*) count FROM ${table} WHERE ${activePredicate} AND (${config} LIKE '%standard%' OR ${id} LIKE 'CARD_STD_%') GROUP BY uid_key HAVING COUNT(*)>1`,[id,config,canonicalUid]);
...
add('A11','critical','Claimed row has no claim audit marker',`SELECT ${id} card_key FROM ${table} WHERE ${source} IN ('claimed','claimed_contact','claimed_personal') AND COALESCE(${claimEvent},'')='' LIMIT 100`,[id,source,claimEvent]);
...
add('A16','critical','Claimed contact lost scanner history',`SELECT ${id} card_key FROM ${table} WHERE ${source} IN ('claimed','claimed_contact') AND COALESCE(${scanner},'')='' LIMIT 100`,[id,source,scanner]);
add('A17','critical','Claimed contact has no recognized/bound claimant pointer',`SELECT ${id} card_key FROM ${table} WHERE ${source} IN ('claimed','claimed_contact') AND COALESCE(${recognized || bound},'')='' LIMIT 100`,[id,source,recognized || bound]);
add('A18','high','Inviter conflicts with original scanner on claim-origin record',`SELECT ${id} card_key FROM ${table} WHERE ${source} IN ('claimed','claimed_contact','claimed_personal') AND COALESCE(${inviter},'')<>'' AND COALESCE(${scanner},'')<>'' AND ${inviter}<>${scanner} LIMIT 100`,[id,source,inviter,scanner]);
add('A19','critical','Contact row is shaped like a personal-card candidate',`SELECT ${id} card_key FROM ${table} WHERE ${contactPredicate} AND (COALESCE(${owner},'')<>'' OR ${id} LIKE 'CARD_STD_%' OR ${id} LIKE 'CARD_VIDEO_%' OR ${id} LIKE 'CARD_SQUARE_%' OR ${id} LIKE 'CARD_POSTER_%') LIMIT 100`,[id,owner]);
add('A20','high','LINE-generated personal exists beside another personal source for same UID',`SELECT COALESCE(${owner},${profile},${line},${bound}) uid_key, COUNT(DISTINCT ${source}) source_count FROM ${table} WHERE ${activePredicate} AND ${personalPredicate} GROUP BY uid_key HAVING source_count > 1 AND SUM(CASE WHEN ${source}='line_generated' THEN 1 ELSE 0 END)>0`,[source,canonicalUid]);

const report = {
  model: 'AI folder retains contact; claim creates/links one personal; inviter/scanner/owner remain separate',
  mode: localDbPath ? 'local-db' : 'schema-snapshot',
  table,
  columns,
  identityColumnsDetected: [line, owner, profile, bound, scanner, creator, inviter, recognized].filter(Boolean),
  warning: 'A15 cross-entry resolver divergence requires trace fixtures. Missing claim/inviter columns are reported as not_evaluable rather than guessed.',
  checks
};
