...
- 最多 8 個商品候選；單一候選且表單擷取欄位仍空白時直接帶入，收合 DM 預覽讓表單可見；多候選或已有手填內容時，保留選擇與確認帶入。最後沿用「儲存商品」，不自動批次寫入、不自動上架。
- 明確組合視為一個販售單位，例如「5入/盒、3盒組、買2送1、原價900、特價600」帶入名稱含3盒組、售價600、說明保留規格與優惠；不除以3，不把900當售價。價格或對應單位確實不明仍留白。
...
- 「同時將 DM 上傳為商品圖片」預設未勾選，勾選表示同意公開可存取圖片；按帶入時沿用 uploadImageToR2。先再次檢查店家；文字先帶入本機草稿，圖片獨立上傳，失敗時保留已帶入文字與原圖片網址並顯示重試提示。取消或未存商品可能留下已同意公開的素材，與既有圖片上傳一致。
