...
- 私人網購人資料支援常用寄送方式 POST／FAMILY／SEVEN 及 store_info。郵寄須填地址；超商須填門市，不強迫提供住宅地址。
- 結帳按「帶入已儲存網購人資料」且勾「同購買人」才帶入寄送方式／門市。另位收件人不被覆蓋，確認送出以本次表單為準。
- Migration 0038 新增欄位；既有資料預設 POST，原訂單快照不變。切換超商品牌時清除前品牌的門市。

## 2026-09-11 網購資格與購買人

- 對照 HookTea worker.js（d8384e76f267082ed1f7d80b2b836a641b126514）的姓名、手機、選填 Email、分欄郵寄地址與超商寄送規格；不移植其會員資料、收款帳號、密鑰或資源。
- 每次 quote 與新建 order 必須含 buyer.name、buyer.phone；buyer.email 選填。手機正規化空白／連字號／+886 後須為 09 開頭 10 碼，Email 填寫時檢查基本格式。
- customer 為獨立收件人姓名與手機。前端預設「收件人同購買人」，可取消後填不同收件人；後端不採用 LINE 暱稱代替必填購買人。
- POST 必填 city、district、address；postal_code 選填，若填須為 3–6 位數字。FAMILY／SEVEN 必填 store_info（手填門市），不呼叫自動選店或物流服務。
- 表單及確認頁保留返回修改；寄送方式切換時只顯示適用欄位。姓名手機留存記憶體，不寫 localStorage；沒有新增 Email 寄信功能。
- buyer 存入既有不可變 snapshot_json，buyer_name 保留相容顯示；訂單詳情只有本人／該店 owner 可讀購買人與收件人。舊訂單無 buyer 仍可查看。
- 限店內商品在 quote 和 order 原子 INSERT 皆拒絕；商品模式變更或購買人資料變更會使舊報價失效。需先套用 0035 再更新 Worker／前端。
- 全站交易開關、各店收款設定與線上點數限制均不變。
