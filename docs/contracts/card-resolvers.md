...
- 2026-09-25 延伸：保留上述舊分數優先、GET 唯讀；獨立已驗證 POST 登記目錄補算工作。只補無舊分数且有公司／職稱摘要者，AI 不接收姓名、UID、電話、Email、私人詳情或標籤。結果放在 `public/member-directory-v1` 專用 intent，不讀其他 public 分數，也不寫入收藏歷史。有效 0 分保留；失敗不假造規則分數。
- 會員工作租約防止重複呼叫；既有每日 AI 配額、分批與失敗退避；既有排程持續查新名單，不擴張為全站互配。每輪重驗本人名片／登入映射／聯絡及封鎖狀態。
- 「最新名單／配對排名」由後端在完整授權且符合文字／業種條件的集合排序後分頁，不只排列前端當頁。缺分數置後，同分按最新名片順序；切換保留文字與業種、重設分頁。
