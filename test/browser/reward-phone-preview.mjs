...
createServer((req,res) => {
  ...
  const role = ['store','redeem'].includes(url.searchParams.get('role')) ? url.searchParams.get('role') : 'reward';
  ...
  window.fetchAPI=async(action,payload)=>{
    ...
    reads++;status();
    if(payload.customerUserId!=='0912345678'&&!(window.userRole==='redeem'&&payload.customerUserId===customer))return {success:false,error:'查無會員，請確認完整手機號碼或改掃會員 QR'};
  ...
  document.getElementById('open').textContent=window.userRole==='redeem'?'開啟扣點':'開啟贈點';
  document.getElementById('open').onclick=()=>openStorePointOperationPopup({mode:window.userRole==='redeem'?'redeem':'reward'});
