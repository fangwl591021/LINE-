...
let apiReads = 0;
const assets = new Map([['/js/modules/member-chat.js', 'text/javascript'], ['/css/member-chat.css', 'text/css']]);
...
createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/fixture-status') { res.writeHead(200, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ apiReads, messages: sql.prepare('SELECT count(*) n FROM member_chat_messages').get().n })); return; }
...
}).listen(8804, '127.0.0.1', () => console.log('Synthetic member chat: http://127.0.0.1:8804/'));
