...
test('mall registration is explicit and lazy and versions match both entrypoints',()=>{
 ...
 assert.match(mall,/case 'registration':[\s\S]*?import\('\.\/store-registration-popup\.js\?v=1'\)/);
 ...
 for(const file of [loader,standalone]){assert.match(file,/store-shop\.js\?v=22/);assert.match(file,/store-shop\.css\?v=18/);}
});
