...
test('main and public pages load the same helper before their consumers; lazy store version is synchronized',()=>{
  ...
  for(const consumer of ['js/modules/store-shop-entry.js?v=35','js/modules/crm.js?v=7.9','js/auth.js?v=11.00'])
    assert.ok(html.indexOf('js/modules/store-invite-route.js?v=1')<html.indexOf(consumer) && html.includes(consumer));
  assert.ok(publicHtml.indexOf('js/modules/store-invite-route.js?v=1')<publicHtml.indexOf('js/modules/store-shop.js?v=33'));
  assert.match(entry,/store-shop\.js\?v=33/);
