...
test('main and public pages load the same helper before their consumers; lazy store version is synchronized',()=>{
  ...
  for(const consumer of ['js/modules/store-shop-entry.js?v=39','js/modules/crm.js?v=7.9','js/auth.js?v=11.02'])
    assert.ok(html.indexOf('js/modules/store-invite-route.js?v=1')<html.indexOf(consumer) && html.includes(consumer));
  assert.ok(publicHtml.indexOf('js/modules/store-invite-route.js?v=1')<publicHtml.indexOf('js/modules/store-shop.js?v=37'));
  assert.match(entry,/store-shop\.js\?v=37/);
