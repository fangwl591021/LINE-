...
const HomeModule = (function() {
    ...
    function buildWeeklyZodiacForecast_(zodiac) {
        const bank = {
            '\u7261\u7f8a\u5ea7': ['主動開局', '本周適合把停在觀望階段的客戶往前推一步。你的優勢在於行動快、決斷清楚，但要避免一開始就太急著成交。先用簡短明確的方案降低對方思考成本，再提出一個容易答應的小行動。', '1. 從收藏名片挑出 3 位最近有互動但尚未約談的人。\n2. 用一句話說明你能幫他解決什麼問題，再附上自己的數位名片。\n3. 對有回應的人安排一次 15 分鐘通話或碰面，不要只停在傳訊息。'],
            ...
            '\u8655\u5973\u5ea7': ['流程校準', '本周適合做資料清理與跟進節奏調整。你只要把名片資料補齊、標籤整理好、下一步排清楚，後續成交率就會提升。避免一邊新增資料、一邊讓舊資料失去管理。', '1. 檢查收藏名片中缺電話、缺公司或缺標籤的資料。\n2. 將重點客戶補上狀態：待聯絡、已回覆、可邀約或暫緩。\n3. 為前 5 位客戶設定下一次跟進日期。'],
    ...
    function renderHomeOnboardingAI_(contacts) {
        ...
        if (!suggestions.length) {
            list.innerHTML = '<div class="home-advice-empty bg-white rounded-[26px] border border-emerald-100 shadow-sm text-[13px] text-emerald-700 font-bold leading-relaxed">目前基礎設定已完成。下一步可以固定每天整理新增名片、追蹤回覆，讓收藏名片變成真正的業務管線。</div>';
            return;
        }
    ...
    window.loadHomeSalesAssistant = async function() {
        ...
        list.innerHTML = '<div class="home-advice-empty bg-white rounded-[26px] border border-slate-100 shadow-sm text-[13px] text-slate-400 font-bold text-center">正在整理今日建議...</div>';
        try {
            ...
            const rows = contacts
                ...
                .slice(0, 3);
            if (!rows.length) {
                list.innerHTML = '<div class="home-advice-empty bg-white rounded-[26px] border border-slate-100 shadow-sm text-[13px] text-slate-400 font-bold text-center">今天沒有待跟進名片。新增名片後，系統會在這裡提醒下一步。</div>';
                return;
            }
