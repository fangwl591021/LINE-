...
const HomeModule = (function() {
    ...
    function renderHomeOnboardingAI_(contacts) {
        ...
        if (!suggestions.length) {
            list.innerHTML = '<div class="bg-white rounded-[26px] border border-emerald-100 shadow-sm p-5 min-h-[104px] flex items-center text-[13px] text-emerald-700 font-bold leading-relaxed">目前基礎設定已完成。下一步可以固定每天整理新增名片、追蹤回覆，讓 AI名片夾變成真正的業務管線。</div>';
            return;
        }
        const toneMap = {
        ...
        list.innerHTML = suggestions.map(function(item, index) {
            ...
            return `
                <button type="button" onclick="${item.onclick}" class="w-full min-h-[112px] bg-white rounded-[26px] border border-pink-100 shadow-sm p-4 text-left active:scale-[0.99] transition-transform flex items-center">
    ...
    window.loadHomeSalesAssistant = async function() {
        ...
        list.innerHTML = '<div class="bg-white rounded-[26px] border border-slate-100 shadow-sm p-5 min-h-[104px] flex items-center justify-center text-[13px] text-slate-400 font-bold text-center">正在整理今日建議...</div>';
        try {
            ...
            const rows = contacts
                ...
                .slice(0, 3);
            if (!rows.length) {
                list.innerHTML = '<div class="bg-white rounded-[26px] border border-slate-100 shadow-sm p-5 min-h-[104px] flex items-center justify-center text-[13px] text-slate-400 font-bold text-center">今天沒有待跟進名片。新增名片後，系統會在這裡提醒下一步。</div>';
                return;
            }
            list.innerHTML = rows.map(item => {
                ...
                return `
                    <button type="button" onclick="window.openCardDetailById ? window.openCardDetailById('${rowId}') : window.goPage('card')" class="w-full min-h-[112px] bg-white rounded-[26px] border border-pink-100 shadow-sm p-4 text-left active:scale-[0.99] transition-transform flex items-center">
                        <div class="flex items-center justify-between gap-3">
                            <div class="min-w-0">
