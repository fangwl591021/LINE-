...
export function mountStorePhoneReward(root,{isCurrent=()=>true,rewardOnly=false,mode='reward'}={}) {
  const debit=mode==='redeem',verb=debit?'扣點':'贈點',pointLabel=debit?'扣除':'贈送';
  if(debit&&window.isRewardOnlyPointCashier?.())throw Error('贈點用戶不能扣點');
  if(!debit&&window.isRedeemOnlyPointCashier?.())throw Error('扣點用戶不能贈點');
  ...
  function recoverDirectSnapshot(saved){
    ...
    if(debit){
      const allowed=['customerUserId','mode','amount','deductPoints','debitPoints'];
      return payload&&Object.keys(payload).every(key=>allowed.includes(key))&&/^U[0-9a-fA-F]{20,64}$/.test(payload.customerUserId||'')&&payload.mode==='redeem'&&payload.amount===0&&Number.isSafeInteger(payload.debitPoints)&&payload.debitPoints>=1&&payload.debitPoints<=1000000&&payload.deductPoints===payload.debitPoints?payload:null;
    }
  ...
  function render(){
    ...
    send.textContent=busy?verb+'處理中…':'確認'+verb;
  ...
  root.querySelector('[data-phone-search]').addEventListener('submit',async event=>{
    ...
    try{
      ...
      if(!/^U[0-9a-fA-F]{20,64}$/.test(id||''))throw Error('此手機尚未綁定可'+verb+'的會員。');
      ...
      member.hidden=false;status.textContent='已找到會員，請核對後輸入'+pointLabel+'點數。';
  ...
  async function submit(snapshot){
    ...
    busy=true;retryAllowed=false;render();status.textContent='正在送出'+verb+'，請勿重複操作…';
    try{
      const result=await window.submitSafeCashier(snapshot);
      if(!current())return;
      if(result?.success!==true)throw Error(result?.error||'未能確認'+verb+'結果，請查詢原交易。');
      completed('已成功'+pointLabel+' '+(snapshot.debitPoints??snapshot.rewardPoints).toLocaleString('zh-TW')+' 點。');
    }catch(error){
      if(!current())return;
      status.textContent=error.message||'未能確認'+verb+'結果，請查詢原交易。';
      try{rememberPending(snapshot);}catch(pendingError){pending={};retrySnapshot=null;status.textContent=pendingError.message;}
      if(pending)status.textContent+=' 請先查詢原交易結果，勿重新建立'+verb+'。';
  ...
  root.querySelector('[data-phone-gift]').addEventListener('submit',async event=>{
    ...
    if(!/^\d+$/.test(raw)||!Number.isSafeInteger(value)||value<1||value>1000000){status.textContent=''+pointLabel+'點數請填 1 至 1,000,000 的整數。';points.focus();return;}
    const snapshot=debit?{customerUserId:customer.id,mode:'redeem',amount:0,debitPoints:value,deductPoints:value}:{customerUserId:customer.id,mode:'reward',amount:value,rewardPoints:value,deductPoints:0,...(rewardOnly?{rewardScanToken:receipt.token}:{})};
  ...
  check.addEventListener('click',async()=>{
    ...
    try{
      ...
      if(['succeeded','failed'].includes(state)){
        completed(result.success?'原交易已完成，請勿重複'+verb+'。':'原交易未完成，請重新搜尋會員後再操作。');
      }else{
        retryAllowed=state==='not_found'&&!!retrySnapshot;
        status.textContent=retryAllowed?'原交易尚未建立，可重送原'+verb+'；會員與點數不會變更。':(result?.displayMessage||result?.error||'原交易尚待確認，請稍後再查詢。');
        if(state==='not_found'&&!retrySnapshot)status.textContent='原交易尚未建立。這不是可恢復的電話'+verb+'，請返回原收銀操作查詢；本視窗不會建立第二筆交易。';
      }
  ...
  if(pending&&!status.textContent)status.textContent=retrySnapshot?'原交易會員：'+retrySnapshot.customerUserId+'；'+pointLabel+' '+(retrySnapshot.debitPoints??retrySnapshot.rewardPoints).toLocaleString('zh-TW')+' 點。請先查詢原交易結果。':'尚有一筆交易等待確認，請先查詢原交易結果。';
