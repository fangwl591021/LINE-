(function() {
  ...
  function load() {
    ...
    pending = new Promise((resolve, reject) => {
      if (!document.getElementById('store-shop-css')) {
        ...
        css.id = 'store-shop-css'; css.rel = 'stylesheet'; css.href = 'css/store-shop.css?v=1';
        document.head.appendChild(css);
      }
      const script = document.createElement('script');
      script.src = 'js/modules/store-shop.js?v=1';
  ...
  window.openStoreShop = async function() {
    ...
    try {
      await load();
      if (window.currentPage === 'store-shop') window.StoreShop.mount(root, false);
    } catch(error) {
      if (window.currentPage !== 'store-shop') return;
      root.querySelector('p').textContent = error.message;
      const retry = document.createElement('button');
      retry.textContent = '重新載入'; retry.onclick = window.openStoreShop; root.appendChild(retry);
    }
  };
})();
