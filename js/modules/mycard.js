...
(function() {
  ...
  function updatePreview() {
    ...
    if (!preview) return;
    preview.className = 'rounded-2xl overflow-hidden shadow-lg border border-slate-100 mb-4 max-w-[420px] mx-auto pointer-events-none bg-white';
    ...
    if (!/^\d+(\.\d+)?\s*\/\s*\d+(\.\d+)?$/.test(ratio)) ratio = layout === 'portrait' ? '2/3' : (layout === 'square' ? '1/1' : '20/13');
