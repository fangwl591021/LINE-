...
export function merchantRole(role) {
  return ({admin:'admin','總管':'admin',store:'store',tenant:'store','店長':'store','租戶':'store',reward:'reward','贈點用戶':'reward'})[String(role||'').toLowerCase()]||'';
}
...
export function renderPointsHome({merchant=false,rewardOnly=false,standalone=false,canManage=false}={}) {
  return `<section class="points-home" aria-label="${merchant?'商家':'消費者'}首頁">
    ${merchant?`<div class="points-merchant-heading"><span class="points-avatar">${pointIcon('bag')}</span><div><h2>${rewardOnly?'贈點工作台':'店家工作台'}</h2><p>${rewardOnly?'掃描會員 QR，核對後贈點':'贈點・折抵・管理'}</p></div><span class="points-role-label">${rewardOnly?'贈點單位':'商家版'}</span></div>`:''}
    <section class="points-wallet" aria-label="本人點數與 QR"><div class="points-wallet-copy"><h2>我的共用點數</h2><strong data-home-balance>${standalone?'登入查看':'讀取點數中…'}</strong><p>依店家規則折抵，不代表現金。</p><button type="button" data-do="wallet" class="points-wallet-refresh">查看／更新點數</button></div><button type="button" data-do="wallet" class="points-wallet-qr" aria-label="放大我的專屬點數 QR"><span data-home-qr>${pointIcon('scan')}</span><strong>我的專屬 QR</strong><small data-home-wallet-status>${standalone?'登入後出示':'正在準備'}</small></button></section>
    <div class="points-actions">${merchant?
      tile('point-reward','gift','贈送點數',rewardOnly?'僅限掃描會員 QR':'送點給會員，增加回購','coral')+(rewardOnly?'':tile('point-redeem','scan','折抵點數','核對會員與折抵金額','blue')):
    ...
    ${merchant?`<div class="points-tools">${tile('spending-history','history','本人紀錄','我的點數明細','mint')}${!rewardOnly?tile('sales','chart','商品業績','商品 QR 折抵','lavender')+tile('online-manage','bag','網路訂單','收款與出貨','sand')+tile('manage','settings','店家設定','店面與商品','gray'):''}</div><p class="points-permission-note">${rewardOnly?'此帳號僅開放掃碼贈點，不提供扣點或店家收款管理。':'贈點與折抵皆須核對會員並確認送出，開啟功能不會直接交易。'}</p>`:''}
