...
export function createBlockSupply(root,{seed=crypto.getRandomValues(new Uint32Array(1))[0],onComplete=()=>{},onFail=()=>{},onExit=()=>{}}={}){
 ...
 root.innerHTML=`<section class="bs-game" data-mode="welcome" aria-label="方塊補給站">
  <header class="bs-header"><div><small>BASE SUPPORT · 本機試玩</small><h1>方塊補給站</h1></div><button data-action="exit">‹ 返回遊戲館</button></header>
  ...
  <div class="bs-footnote">90 秒補滿 100 能源 · 本機試玩不發點</div>
 </section>`;
 ...
 const resultCopy='<p class="bs-local">本機試玩，尚未串接每日獎勵；本局成績尚未上傳。</p>';
 function finish(){
 ...
 function restart(){if(dead)return;stopLoop();audio.stop();s=createGame(seed);acc=0;replay=[];finished=false;mode='welcome';paint();start();}
