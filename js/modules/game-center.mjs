...
function styles(){for(const name of ['game-center','block-supply'])if(!document.querySelector(`[data-game-style="${name}"]`)){const l=document.createElement('link');l.rel='stylesheet';l.href=new URL(`../../css/${name}.css?v=1`,import.meta.url).href;l.dataset.gameStyle=name;document.head.append(l);}}
function ensure(){
 ...
 dialog.addEventListener('click',e=>{const b=e.target.closest('[data-gc]');if(!b)return;const action=b.dataset.gc;if(action==='close')close();if(action==='refresh')void refresh();if(action==='play')void play(b.dataset.game);if(action==='confirm')void confirm();});
}
...
async function play(gameId,replay=false){
 ...
 $('[data-gc-status]').textContent='正在準備補給任務…';controller?.setRewardStatus('正在準備下一局…');
 try{
  const [module,session]=await Promise.all([import('./block-supply-controller.mjs?v=3'),gameAPI('startGame',{gameId,replay,deviceType:deviceType()})]);
  if(version!==generation||!current())return;
  controller?.destroy();currentSession=session;$('.gc-lobby').hidden=true;host.hidden=false;
  const done=(_result,proof)=>{if(member!==gameUser())return;pendingGame({sessionId:session.sessionId,gameId,nonce:session.nonce,replay:proof.replay},member);void confirm();};
  controller=module.createBlockSupply(host,{seed:session.seed,localPreview:false,isCurrent:()=>member===gameUser()&&current(),onComplete:done,onFail:done,onRestart:()=>void play(gameId,true),onExit:()=>{void gameEvent('game_exit',session);lobby();},onEvent:type=>{if(type==='confirm')void confirm();else void gameEvent(type,session);}});
