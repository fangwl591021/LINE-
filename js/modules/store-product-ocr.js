...
export function openProductDm(form,{base,prepareImage,isCurrent}){
  ...
  let serial=0,busy=false,controller,dmImage='',candidates=[];
  ...
  result.append(label,details,warning,imageLabel,apply);
  panel.append(title,note,choose,close,picker,status,preview,result);
  ...
  picker.onchange=async()=>{
    ...
    try{
      ...
      select.value='0';showCandidate();result.hidden=false;status.textContent='已辨識 '+candidates.length+' 項，請選擇並帶入表單。多於 8 項請分張辨識。';
  ...
  apply.onclick=async()=>{
    ...
    if(['title','description','price','category'].some(key=>fields[key]?.value)&&!window.confirm('將以辨識結果替換商品名稱、說明、價格與分類。銷售方式、折抵規則及上架狀態保持不變。是否帶入？'))return;
    ...
    try{
      ...
      if(useImage.checked){
        ...
        if(!uploaded?.success||!uploaded.url)throw Error('DM 圖片上傳失敗；原商品表單未更動');
        const url=new URL(uploaded.url);if(url.protocol!=='https:'||url.username||url.password)throw Error('圖片網址無效');
        imageUrl=url.href;
      }
      if(!current()||id!==serial)return;
      fields.title.value=p.title;fields.description.value=p.description;
      fields.price.value=p.price_cents===null?'':String(p.price_cents/100);
      ...
      fields.title.focus();
    }catch(error){if(current()&&id===serial)status.textContent=error.message;}
