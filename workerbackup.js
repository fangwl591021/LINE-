  ...
  async recognize(payload, env) {
    try {
      const uploadedImgUrl = payload.deferImageUpload === true
        ? ''
        : await StorageModule.upload(payload.base64Image, env);
      const prompt = [
        '請解析這張名片並提取資訊，同一次完成 OCR 與名片外框定位。支援多國語言（如英文、日文等），若無中文請直接保留原文。',
        '只回傳純 JSON，不要 markdown：',
        '{"isBusinessCard":true,"reason":"","confidence":0.95,"cardLocalization":{"detected":true,"incomplete":false,"cropConfidence":0.95,"boundingBox":{"x":0.05,"y":0.18,"width":0.9,"height":0.58},"corners":[{"x":0.05,"y":0.18},{"x":0.95,"y":0.18},{"x":0.95,"y":0.76},{"x":0.05,"y":0.76}],"clippedEdges":[]},"姓名":"","英文名":"","職稱":"","公司名稱":"","手機號碼":"","公司電話":"","電子郵件":"","公司網址":"","公司地址":"","統一編號":"","分機":"","傳真":"","部門":"","社群帳號":"","服務項目":""}',
        'cardLocalization 規則：',
        '1. 所有座標都使用整張輸入圖片的 0 到 1 正規化座標。boundingBox 必須只包住真實名片，不可包入桌面、手掌、鍵盤或其他背景。',
        '2. corners 固定依序回傳左上、右上、右下、左下四點。',
        '3. 若任一真實名片邊緣超出照片或無法確認，incomplete=true，並在 clippedEdges 列出 left/right/top/bottom；不可推測不存在的邊界。',
        '4. cropConfidence 是名片邊界定位信心，不確定時降低分數，不可硬猜。',
        '5. 若圖片不是名片，isBusinessCard=false 並填 reason。',
        '所有名片欄位必須是字串，保留開頭的 0。若名片上沒有清楚服務項目，請依公司名稱、職稱、地址、網站合理推估，替「服務項目」生成 3 到 5 行簡短商務介紹；每行 12 到 18 字，避免誇大，不要寫「未知」。'
      ].join('\n');
      ...
      if (cardData && cardData.isBusinessCard === false) {
        throw new Error(String(cardData.reason || '這張圖片看起來不是名片，請重新拍攝'));
      }
      const localization = cardData && cardData.cardLocalization && typeof cardData.cardLocalization === 'object'
        ? cardData.cardLocalization
        : null;
      const confidence = Math.max(0, Math.min(1, Number(cardData && cardData.confidence) || 0));
      delete cardData.isBusinessCard;
      delete cardData.reason;
      delete cardData.confidence;
      delete cardData.cardLocalization;
      
      if (uploadedImgUrl) cardData['名片圖檔'] = uploadedImgUrl;
      ...
      return { success: true, data: { cardData, localization, confidence } };
    } catch (e) {
